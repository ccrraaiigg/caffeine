document.extension = true

// We shouldn't be using setTimeout() anywhere...
setTimeout(
  function () {
    // Attempt to keep this page embeddable in external non-secure origins.
    var magicWindow,
    width = window.innerWidth,
    height = window.innerHeight
    
    try {magicWindow = window.top.magicWindow}
    catch (exception) {magicWindow = null}

    window.document.ontouchend = (event) => {
      if (!window.muteButtonOverridden) {
        // Unlock HTML5 Audio - load a data url of short silence and play it
        // This will allow us to play web audio when the Apple iPhone/iPad hardware
        // mute toggle is on.
      
        var silenceDataURL = "data:audio/mp3;base64,",
            tag = document.createElement("audio")
        
        tag.controls = false
        tag.preload = "auto"
        tag.loop = false
        tag.src = silenceDataURL

        tag.onended = () => {
          window.muteButtonOverridden = true
          console.log("iOS hardware mute switch overridden")}

        var p = tag.play()

        if (p) p.then(
          () => {},
          (reason) => {console.log("iOS hardware mute switch not overridden, HTML5 audio play failed", reason)})}
      
      window.document.getElementById("Caffeine").contentWindow.focus()
      window.document.getElementById("embeddedSqueak").style.zIndex = 1000
      window.document.getElementById("embeddedSqueak").style.boxShadow = "1px 1px 7px #999, 2px 2px 8px #999, 3px 3px 9px #999"}

//    window.document.getElementById("embeddedSqueak").ontouchend = window.document.ontouchend
//    window.document.getElementById("Caffeine").ontouchend = window.document.ontouchend

    if (magicWindow) {
      magicWindow.squeakDisplay = window.startCaffeine(
        squeak,
        "caffeine",
        "SqueakV46",
        {
        width: width,
        height: height,
        appID: "4599d316-a13e-46ef-92be-c7337899038c",
        appName: "Caffeine",
        appServer: "demo.blackpagedigital.com:8091"})}
    else {
      window.startCaffeine(
        squeak,
        "caffeine",
        "SqueakV46",
        {
        width: width,
        height: height,
        appID: "4599d316-a13e-46ef-92be-c7337899038c",
        appName: "Caffeine",
        appServer: "demo.blackpagedigital.com:8091"})}},
  2000)